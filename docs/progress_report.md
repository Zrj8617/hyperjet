...
## 12. 延长训练验证：89 次更新的平台持久性（2026-08-10）

### 12.1 目的

同事质疑"30 次更新就收敛不可能"。为了证明 30 次到达的平台是真实收敛点而非
"训练不足的假象"，把同一配置的训练延长到更多更新，观察平台是否持续。

### 12.2 配置与说明

- 采样配置不变：`--num-envs 4 --rollout-horizon 200`（每次更新 = 4 环境 ×
  完整一集 = 800 时隙）；
- 原计划 100 次更新 × 3 epoch，因单次会话时长限制改为 **100 次 × 1 epoch**
  （总梯度步数 89 ≈ 原 30×3=90 步，训练量相当）；实际因超时完成 **89 次
  更新 = 356 集 = 71,200 时隙**（原 30 次运行总训练量的约 3 倍）；
- 运行目录：`logs/clean_mainline/20260809_232448_formal_mlp_4env_h200_100up_seed0`。

### 12.3 结果：平台从 update 51 一直保持到 89

| 更新段 | reward 均值 | std | 阶段 |
|---:|---:|---:|---|
| 1~10 | −0.05 | 0.15 | 起点 |
| 11~20 | 0.40 | 0.49 | 上升 |
| 21~50 | 4.41 | 1.59 | 快速上升 |
| **51~89** | **6.13** | **0.24** | **平台（39 次更新不动）** |

![89 次更新原始 reward 曲线](figures/mlp_4env_h200_89up_reward_raw.png)

同时段 DAG 完成率稳定 0.91~0.99，流时间 ~60s。

### 12.4 结论

1. **收敛真实**：奖励在 update 51 后持续 39 次更新保持 6.13 ± 0.24，既无继续
   上升（排除"没学完"）也无发散（排除"不稳定"）；
