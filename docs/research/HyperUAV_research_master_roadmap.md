...
## 2026-09-15 B2 账本闭合声明被证伪（六臂门禁 G9）

**结论：交接文档 §4.4 的"B2 三段账 = B1 实际 flowtime、两者数学上相等"是错的，从未成立。**

- 同轨迹实测最大相对误差 **0.4891791541**（门禁要求 <1e-6），`b2_total ≈ 0.51 × b1_total`。
- 机制：闭合要求 `Σ ΔΦ_G = anchor_G − T̂⁰_G`，即预测漂移全部由分配决策造成。实际上预测还会
  因 UE 移动、新 DAG 到达、以及影子 DP 的贪心+乐观假设而漂移，这部分**未被任何一项记账**。
  已排除"两侧记账 DAG 集合不同"这一替代解释（`environment/forecast.py:198` 覆盖全部未完成 job）。
- **作废的推论**：B1/B2 优化同一目标函数；"两者都差 ⇒ 问题在目标函数本身"。
- **坐实的结论**：B2 奖励中约一半的量来自策略无法控制的预测漂移。这是 §4.4"B2 弱的机制假说"
  的直接证据，且强于原假说（原假说只说 `initial_cost` 量级大）。
- **口径影响**：`B2 − B1` 是两个不同目标函数的对比，不得表述为"同一实际代价的分期记账效应"。
- 本轮未启动任何正式训练或评测；6 个 5-episode smoke 的 G1–G8 与 flag/回归检查全部通过。
- 来源：`docs/superpowers/reports/2026-09-15-six-arm-realign-gating-results.md`
  （实现 commit `e7056da`，报告 commit `3aa4b93`）
- 方法论后果：已写入 `AGENTS.md` §4 第 11 条——文档里的定量断言在使用前必须验证。

### 2026-09-15 补充：G9-3 复测通过，闭合缺口的成因已定量坐实

修订后的 G9-3 判定 **PASS**（报告 commit `6722311`，服务器 HEAD `70c862a`）：

- 5/5 episode 的 DAG 记账集合**完全相同**（例：episode 0 为 190 vs 190、未完成 46 vs 46，
  四个差集均为空）→ 彻底排除"两侧记的 DAG 不是同一批"
- `b1_minus_b2` 与 `unbooked_prediction_drift` 逐值相等，最大残差 **8.73e-11 秒**，
  漂移对缺口的解释率 **≈100%**
- 方向：`b2_total < b1_total`（episode 0：49,812 s vs 97,514 s）
- **可写进论文的关键数字**：`Σ ΔΦ = 35,861 s`，而预测总漂移
  `Σ(anchor − T̂⁰) = 83,563 s` —— **只有约 43% 的预测漂移由分配决策造成，约 57% 是外生的**
  （UE 移动、新 DAG 到达、影子 DP 的贪心+乐观偏差）

这把 §4.4 原本"仍未坐实"的 B2 失败机制假说升级为**定量结论**：
B2 的奖励里有一半以上的量是任何策略都无法影响的。
