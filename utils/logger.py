...
class Log:
    def __init__(self) -> None:
        ...
        self.extra_metrics: dict[str, list[float | None]] = {}
    ...
    def append(
        ...
        *,
        ...
        for key in list(self.extra_metrics.keys()):
            if key not in extra_metrics:
                self.extra_metrics[key].append(None)

        for key, value in extra_metrics.items():
            if key not in self.extra_metrics:
                self.extra_metrics[key] = [None] * (current_length - 1)
...
class Logger:
    ...
    def log_metrics(
        ...
        aggregated_extra_metrics: dict[str, float] = {}
        if extra_metrics is not None:
            aggregated_extra_metrics = {key: float(value) for key, value in extra_metrics.items()}
        else:
            for key, values in log.extra_metrics.items():
                vals = [x for x in values[-log_freq:] if x is not None]
                if vals:
                    aggregated_extra_metrics[key] = float(np.mean(np.array(vals)))
        ...
        for key, value in aggregated_extra_metrics.items():
            loss_parts.append(f"{key}: {value:.3f}")
