...
import config
from marl_models.hgnn.layers import EdgeAggregationLayer, HyperedgePoolingLayer
...
class PhaseOneHGNNEncoder(nn.Module):
    ...
    def __init__(self, task_input_dim: int, uav_input_dim: int, hidden_dim: int, num_layers: int) -> None:
        ...
        self.task_hyper_layers = nn.ModuleList([HyperedgePoolingLayer(hidden_dim) for _ in range(num_layers)])
    ...
    def forward(
        ...
        for layer_idx in range(len(self.task_dep_layers)):
            ...
            task_hidden = self.task_hyper_layers[layer_idx](task_hidden, task_hyperedges)
