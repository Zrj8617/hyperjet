...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--min-decision-order", type=int, default=0)
    parser.add_argument("--source-decisions", type=Path)
    parser.add_argument("--multi-root-seeds", type=int, nargs="+", default=(0, 1, 2))
    parser.add_argument("--audit-seeds", type=int, nargs="+", default=(0, 1, 2))
    parser.add_argument("--audit-updates", type=int, nargs="+", default=(30, 60, 120))
...
def run_pilot(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if int(args.decisions_per_checkpoint) != 3 and not (
        args.mode == "sequential-root-cause"
        and int(args.decisions_per_checkpoint) == 1
    ):
        raise ValueError("the formal ranking pilot is capped at 3 decisions/checkpoint")
    phase_by_update = {30: "early", 60: "mid", 120: "late"}
    selected_seeds = tuple(int(value) for value in args.audit_seeds)
    selected_updates = tuple(int(value) for value in args.audit_updates)
    if any(seed not in (0, 1, 2) for seed in selected_seeds):
        raise ValueError("audit seeds must be selected from 0/1/2")
    if any(update not in phase_by_update for update in selected_updates):
        raise ValueError("audit updates must be selected from 30/60/120")
    root_cause = args.mode in ("root-cause", "sequential-root-cause")
    sequential = args.mode == "sequential-root-cause"
    min_decision_order = max(int(args.min_decision_order), 1) if sequential else 0
    ...
    for seed in selected_seeds:
        ...
        for update in selected_updates:
            ...
            rows = _audit_checkpoint(
                ...
                min_decision_order=min_decision_order,
            )
    ...
    prefix = (
        "sequential_root_cause"
        if sequential
        else ("root_cause" if root_cause else "pilot")
    )
...
def run_multi_root(args: argparse.Namespace) -> dict[str, Any]:
    ...
    sequential = args.mode == "sequential-multi-root"
    if not sequential and len(source_rows) != 27:
        raise ValueError("multi-root diagnostic requires the existing 27 decisions")
    if sequential and not source_rows:
        raise ValueError("sequential multi-root diagnostic requires source decisions")
    ...
    prefix = "sequential_multi_root" if sequential else "multi_root"
    decision_path = args.output / f"{prefix}_decisions.jsonl"
    summary_path = args.output / f"{prefix}_summary.json"
    decision_path.write_text(
        "".join(json.dumps(row, sort_keys=True) + "\n" for row in decisions),
        encoding="utf-8",
    )
...
def _multi_root_one_decision(
    *,
    ...
    row = {
        ...
        "legal_q_inputs": list(source.get("legal_q_inputs", [])),
        "candidate_features": list(source.get("candidate_features", [])),
        "critic_global_context": list(source.get("critic_global_context", [])),
        "state_scalar_suffix": dict(source.get("state_scalar_suffix", {})),
        "sequential_reservation_audit": dict(
            source.get("sequential_reservation_audit", {})
        ),
        ...
        for _ in range(max_search_slots):
            ...
            if len(audited) < decision_limit:
                for target_trace in trace:
                    if int(target_trace.decision_order) < int(min_decision_order):
                        continue
        ...
        row.update(
            {
                ...
                "sequential_reservation_audit": _sequential_reservation_audit(
                    state=state,
                    snapshot=snapshot,
                    trace=trace,
                    target_trace=target_trace,
...
def _reservation_snapshot(state: TemporaryReservationState) -> dict[str, Any]:
    return {
        "queue_lengths": {str(key): int(value) for key, value in sorted(state.queue_lengths.items())},
        "available_times": {
            str(key): float(value) for key, value in sorted(state.available_times.items())
        },
        "queued_workloads": {
            str(key): float(value) for key, value in sorted(state.queued_workloads.items())
        },
        "slot_assigned_counts": {
            str(key): int(value) for key, value in sorted(state.slot_assigned_counts.items())
        },
        "reserved_task_ids": sorted(str(value) for value in state.reserved_task_ids),
    }


def _sequential_reservation_audit(
    *,
    state: CleanDecisionState,
    snapshot: Any,
    trace: Any,
    ...
    """Rebuild only the existing sequential reservation feature path."""
    reservation = TemporaryReservationState.from_executor(snapshot.uavs, snapshot.executor)
    slot_start = _reservation_snapshot(reservation)
    task_embeddings = np.asarray(encoded.task_embeddings.detach().cpu(), dtype=np.float32)
    ordered_trace = tuple(sorted(trace, key=lambda item: int(item.decision_order)))

    def components(trace_row: Any) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[int], Any]:
        task = snapshot.task_manager.get_task(str(trace_row.task_id))
        if task is None:
            raise ValueError("reservation audit lost a baseline task")
        dynamic, pair, mask, candidate_ids, estimates = build_offloading_candidate_components(
            task=task,
            uavs=snapshot.uavs,
            task_manager=snapshot.task_manager,
            executor=snapshot.executor,
            state_view=reservation,
            current_time_seconds=float(snapshot.current_time_seconds),
            uav_service_positions=snapshot.uav_service_positions,
            ue_service_positions=snapshot.ue_service_positions,
            ues=snapshot.ues,
        )
        ...
        return features, np.asarray(dynamic), np.asarray(pair), [int(x) for x in candidate_ids], (mask, estimates)

    (
        target_slot_start_features,
        target_slot_start_dynamic,
        target_slot_start_pair,
        target_ids,
        target_slot_start_extra,
    ) = components(target_trace)
    target_mask, _ = target_slot_start_extra
    prefix_rows = []
    for trace_row in ordered_trace:
        if int(trace_row.decision_order) == int(target_trace.decision_order):
            break
        features, dynamic, pair, candidate_ids, extra = components(trace_row)
        mask, estimates = extra
        selected_uav_id = int(trace_row.selected_uav_id)
        selected_action = candidate_ids.index(selected_uav_id)
        if not bool(mask[selected_action]):
            raise AssertionError("reservation prefix replay became infeasible")
        estimate = estimates[selected_action]
        reservation.reserve(
            str(trace_row.task_id),
            selected_uav_id,
        ...
        prefix_rows.append(
            {
                ...
                "task_id": str(trace_row.task_id),
                "selected_uav_id": selected_uav_id,
            }
        )
    before_current = _reservation_snapshot(reservation)
    current_features, current_dynamic, current_pair, current_ids, extra = components(target_trace)
    current_mask, current_estimates = extra
    if current_ids != [int(value) for value in state.candidate_uav_ids]:
        raise AssertionError("reservation audit candidate IDs changed")
    if not np.array_equal(np.asarray(current_mask, dtype=bool), state.candidate_mask):
        raise AssertionError("reservation audit legal mask changed")
    current_feature_error = float(
        np.max(np.abs(current_features - np.asarray(state.candidate_features, dtype=np.float32)))
    )
    if current_feature_error > 1e-6:
        raise AssertionError("reservation audit did not reconstruct candidate features")

    selected_action = current_ids.index(int(target_trace.selected_uav_id))
    selected_estimate = current_estimates[selected_action]
    reservation.reserve(
        str(target_trace.task_id),
        int(target_trace.selected_uav_id),
    ...
    after_current = _reservation_snapshot(reservation)
    feature_delta = current_features - np.asarray(target_slot_start_features, dtype=np.float32)
    return {
        "prefix_decision_count": len(prefix_rows),
        "prefix_assignments": prefix_rows,
        "slot_start_reservation": slot_start,
        "before_current_decision_reservation": before_current,
        "after_current_decision_reservation": after_current,
        "candidate_uav_ids": current_ids,
        "slot_start_candidate_features": np.asarray(target_slot_start_features, dtype=np.float32).tolist(),
        "before_current_candidate_features": current_features.tolist(),
        "slot_start_dynamic_features": np.asarray(target_slot_start_dynamic, dtype=np.float32).tolist(),
        "before_current_dynamic_features": current_dynamic.astype(np.float32).tolist(),
        "slot_start_pair_features": np.asarray(target_slot_start_pair, dtype=np.float32).tolist(),
        "before_current_pair_features": current_pair.astype(np.float32).tolist(),
        "candidate_feature_delta_l2": float(np.linalg.norm(feature_delta)),
        "candidate_feature_delta_max_abs": float(np.max(np.abs(feature_delta))),
        "candidate_feature_changed_count": int(np.count_nonzero(np.abs(feature_delta) > 1e-7)),
        "current_state_reconstruction_max_abs_error": current_feature_error,
        "slot_start_legal_mask": np.asarray(target_mask, dtype=bool).tolist(),
        "before_current_legal_mask": np.asarray(current_mask, dtype=bool).tolist(),
    }
...
def _reconstruct_one_forced_action_target(
    *,
        ...
        reconstructed_trace.append(
            {
                ...
                "decision_order": int(trace_row.decision_order),
                "selected_uav_id": int(selected_uav_id),
                "candidate_uav_ids": candidate_ids,
                "candidate_mask": np.asarray(mask, dtype=bool).tolist(),
            }
        )
        if int(trace_row.decision_order) == int(target_trace.decision_order):
    ...
    next_legal_q_vector: list[float] = []
    next_legal_behavior_probabilities: list[float] = []
    next_legal_q_inputs: list[list[float]] = []
    exact_next_state: dict[str, Any] | None = None
    next_q_spread = 0.0
    if not target_transition.terminated:
        ...
        next_legal_q_vector = [float(value) for value in next_q]
        next_probabilities = np.asarray(
            target_transition.next_state.old_masked_probabilities, dtype=np.float64
        )[np.asarray(next_legal_indices, dtype=np.int64)]
        next_probabilities /= next_probabilities.sum()
        next_legal_behavior_probabilities = [
            float(value) for value in next_probabilities
        ]
        next_legal_q_inputs = np.asarray(next_inputs, dtype=np.float32).tolist()
        exact_next_state = {
            "task_id": str(target_transition.next_state.task_id),
            "decision_order": int(target_transition.next_state.decision_order),
            "candidate_uav_ids": [
                int(value) for value in target_transition.next_state.candidate_uav_ids
            ],
            "candidate_mask": np.asarray(
                target_transition.next_state.candidate_mask, dtype=bool
            ).tolist(),
            "candidate_features": np.asarray(
                target_transition.next_state.candidate_features, dtype=np.float32
            ).tolist(),
            "critic_global_context": np.asarray(
                target_transition.next_state.critic_global_context, dtype=np.float32
            ).reshape(-1).tolist(),
            "old_masked_probabilities": np.asarray(
                target_transition.next_state.old_masked_probabilities,
                dtype=np.float32,
            ).tolist(),
        }
    ...
    return (
        {
            ...
            "next_legal_q_vector": next_legal_q_vector,
            "next_legal_behavior_probabilities": next_legal_behavior_probabilities,
            "next_legal_q_inputs": next_legal_q_inputs,
            "exact_next_state": exact_next_state,
            "reconstructed_trace_through_next": reconstructed_trace,
...
def _root_cause_summary(
    ...
    for phase, update in phase_updates.items():
        ...
        if not phase_rows:
            continue
        by_seed = {}
        phase_seeds = sorted({int(row["seed"]) for row in phase_rows})
            ...
            "pooled": {
                **_three_layer_calibration(phase_rows),
                ...
                "training_q_ev_pre_mean": _mean_or_none(
                    [ev_rows[(seed, update)]["training_q_ev_pre"] for seed in phase_seeds]
                ),
                "training_q_ev_post_mean": _mean_or_none(
                    [ev_rows[(seed, update)]["training_q_ev_post"] for seed in phase_seeds]
                ),
            },
        }
    metrics = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    elif args.mode in ("multi-root", "sequential-multi-root"):
